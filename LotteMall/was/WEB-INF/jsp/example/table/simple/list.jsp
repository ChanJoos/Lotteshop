...
<script language="javascript">
		...
		var config = {
			    columns:[/* 컬럼에 대한 내용을 세팅한다. */
			             {name:"RGB",type:"image"}
				          ...
				          ,{name:"MONEY4",type:"float",pointIndex:2}
	...
	function fn_doSearch() {
		...
		*/
		...
		callJson("/phone/example/table/simple/list.moJson", params, fnNmGetter().name  );
	}
...
<div class="subLayout">
	<div class="wrap">
		...
		<div id="content" class="scroll_y">
			...
			<div class="topCont">
				<p class="date"><input type="text" value="2013년 7월" readonly="readonly" /></p>	
			...
			<form:form commandName="sheetSimpleVO" id="listForm" name="listForm" method="post">
				...
				<table id="mySheet" class="tbl th_al" summary="부분별 실적의 SM본부, SI부문, 하이테크부문, BSP부문, ISC본부, 정보기술연수소의 매출/이익에 대한 표입니다.">
					...
					<tfoot id="mySheet_SumRow" style="display:none">
						<tr>
							<th scope="col">합계</th>
							<td class="ar size1"><span>{SUM_MONEY1}</span>억원</td>
							<td class="ar size2"><span class="bl">{SUM_MONEY2}</span>억원</td>
							<td class="ar size1"><span>{SUM_MONEY3}</span>억원</td>
							<td class="ar size2"><span class="bl">{SUM_MONEY4}</span>억원</td>
						</tr>
					</tfoot>
