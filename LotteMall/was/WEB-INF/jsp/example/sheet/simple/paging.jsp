...
<script language="javascript">
	...
	function fn_LoadPage() {
		...
		fn_initSheet();
		fn_doSearch();
		//화면 로딩시 페이지 번호가 1이 아니면 아래와 같이 넣어 주면 됩니다.
		//fn_doSearch( 페이지 번호 );
	}
	...
	function fn_initSheet() {
		var cfg = {SearchMode : 1,Page : $("mySheet_rowPerPage").val(),MergeSheet:msHeaderOnly};
		...
		/* 시트 폰트 크기 및 기타 설정 */
		SheetUtil.initSheet( "mySheet" , mySheet  ,{cfg:cfg,headers:headers,cols:cols,pageAreaID:"mySheetPagingIndex" });
		/* 시트 폰트 크기 및 기타 설정 */
	}
	...
	function mySheet_OnSearchEnd(cd, msg) {
		//조회 후 처리 시 사용		
		sheetLoadEnd = true;
		SheetUtil.makePageIndex("mySheet");
	}
	...
	function mySheet_GoToPage( pageNo ){
		fn_doSearch( pageNo );
	}
	...
	function fn_doSearch( pageNo ) {
		...
		sheetLoadEnd = false;
		mySheet.DoSearch("/phone/example/sheet/simple/paging.moSheet", param);
	}
</script>
<style>
</style>
			...
			<div id="content">
				...
				<div class="subTabWrap">
					...
					<form:form commandName="sheetSimpleVO" id="frmDefault" name="listForm"
						...
						<input type="hidden" name="mySheet_rowPerPage" id="mySheet_rowPerPage" value="10"/>
